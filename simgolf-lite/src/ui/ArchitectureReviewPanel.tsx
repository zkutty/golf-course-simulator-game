...
export function ArchitectureReviewPanel(props: {
  ...
  >
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
      ...
      <label>{t("architecture.review.tee")}<select data-testid="architecture-tee-filter" value={props.review.filters.teeSet} onChange={(event) => set("teeSet", event.target.value as ArchitectureReviewFilters["teeSet"])}>{(["all", "forward", "member", "championship"] as const).map((value) => <option value={value} key={value}>{value === "all" ? t("architecture.review.all") : value}</option>)}</select></label>
      <label>{t("architecture.review.pin")}<select data-testid="architecture-pin-filter" value={props.review.filters.pinRotation} onChange={(event) => set("pinRotation", event.target.value as ArchitectureReviewFilters["pinRotation"])}>{(["A", "B", "C", "all"] as const).map((value) => <option value={value} key={value}>{value === "all" ? t("architecture.review.all") : value}</option>)}</select></label>
