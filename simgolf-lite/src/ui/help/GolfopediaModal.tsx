...
export function GolfopediaModal(props: { open: boolean; onClose: () => void; initialEntry?: string | null }) {
  const initial = GOLFOPEDIA_ENTRIES.find((entry) => entry.id === props.initialEntry);
  ...
  const selected = GOLFOPEDIA_ENTRIES.find((entry) => entry.id === selectedId) ?? filtered[0] ?? GOLFOPEDIA_ENTRIES[0];
      ...
      <div ref={trapRef} style={{ width: "min(980px, 100%)", height: "min(720px, 92vh)", display: "flex", flexDirection: "column", overflow: "hidden", borderRadius: 18, border: "2px solid #3d4a3e", background: "#f7f0dd", boxShadow: "0 28px 80px rgba(0,0,0,.4)" }} onClick={(event) => event.stopPropagation()}>
        <header style={{ padding: 18, borderBottom: "1px solid rgba(61,74,62,.25)", background: "#e8dcc0" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 12 }}>
            ...
            <button onClick={props.onClose} aria-label="Close Golfopedia" style={{ border: "1px solid #687266", borderRadius: 8, background: "#fffaf0", padding: "8px 11px", cursor: "pointer", fontWeight: 900 }}>✕</button>
          </div>
          <div style={{ display: "grid", gap: 10, minWidth: 0 }}>
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search terrain, stats, controls…" aria-label="Search Golfopedia" style={{ width: "100%", minWidth: 0, boxSizing: "border-box", borderRadius: 999, border: "1px solid rgba(61,74,62,.35)", padding: "11px 15px", background: "#fffdf6" }} />
          </div>
        </header>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(190px, 30%) 1fr", minHeight: 0, flex: 1 }}>
          <nav style={{ overflowY: "auto", borderRight: "1px solid rgba(61,74,62,.2)", padding: 10 }}>
            ...
            {filtered.length === 0 && <div style={{ padding: 12, color: "#6b7280", fontSize: 13 }}>No entries match that search.</div>}
          </nav>
