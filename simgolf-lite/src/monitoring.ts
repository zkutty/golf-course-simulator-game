...
export { resolveSentryEnvironment, sanitizeSentryEvent } from "./sentryPrivacy";
...
const pendingAppErrors: Array<{ error: Error; info: ErrorInfo }> = [];
...
function initializeSentry(dsn: string): Promise<void> {
  if (captureAppError) return Promise.resolve();
  ...
  sentryInitialization = import("./sentryRuntime").then(({ initializeSentryRuntime }) => {
    captureAppError = initializeSentryRuntime(dsn);
    for (const { error, info } of pendingAppErrors.splice(0)) captureAppError(error, info);
  }).catch(() => {
...
export function reportAppError(error: Error, info: ErrorInfo): string | undefined {
  ...
  pendingAppErrors.push({ error, info });
