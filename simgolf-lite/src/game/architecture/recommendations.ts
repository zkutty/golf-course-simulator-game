...
function cost(course: Course, hole: M48StrategicHoleEvaluation, factor: number): number {
  const tiles = Math.max(1, hole.options[0]?.geometry.length ?? 1);
  return Math.round(tiles * Math.max(1, course.yardsPerTile) * factor);
}

function recommendationFor(course: Course, hole: M48StrategicHoleEvaluation): M48Recommendation[] {
