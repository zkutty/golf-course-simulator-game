...
import type { TournamentEntrant, TournamentEvent, TournamentTier } from "./types";
...
export function createTournamentEvent(args: { course: Course; world: World; tier: TournamentTier; currentDay: number; daysAhead: number; courseId?: string }): { ok: true; event: TournamentEvent } | { ok: false; reason: string } {
  ...
  const field: TournamentEntrant[] = Array.from({ length: spec.fieldSize }, (_, index) => ({ id: `entrant-${seed}-${index + 1}`, name: golferName(rng(), rng()), archetype: archetypeForTier(args.tier, rng()), skill: spec.skillMin + rng() * (spec.skillMax - spec.skillMin) }));
