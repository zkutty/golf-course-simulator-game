...
- Packet C now derives four source-aware M19 groups as three rendered zones (9, 10, and a seam-free merged 18-source field) from Chebyshev-4 real-tree clusters, preserves real props as authority, excludes non-source obstacles, and applies the camera-aware `P * camera * D4 * P^-1` renderer affine. The 8-transform × 4-rotation matrix checks and focused tests pass with TypeScript, scoped lint, and diff checks.
- The final evidence-based visual replan renders habitat ground beneath source trees and bushes while retaining rock and unrelated-obstacle exclusion, and widens disconnected-side cutback for fuller irregular boundaries. Independent technical review accepts the source/authority policy; independent visual review accepts the fresh M19/detail captures at 3/4 for density, continuity, and tree/habitat integration with no release blocker. The official two-iteration web-game client produced identical structured snapshots and no error artifact.
- Certification exposed two distinct gaps and registered them in Linear. ZK-1241 synchronizes the minimap's collapsed state when entering Operate Cozy mode. ZK-1244 shards the unchanged 24-view ZK-1237 matrix by viewport after the monolithic node exhausted its three-attempt timeout loop. Compact 12/12 passes in 3.8 minutes and desktop 12/12 in 9.1 minutes; their reports share the exact commit, diff, and source fingerprint, contain zero runtime errors, and mechanically cover 24/24 unique viewport/pin/rotation combinations under `/private/tmp/zk1240-full-matrix-sharded`.
- Exact-candidate gates pass: full CI 266 files / 2,123 passed / one intentional skip plus audio audit 5/5; TypeScript; lint/i18n with zero errors and nine inherited Hook warnings; production build and all asset/residency/delivery audits; PWA strict-CSP/offline/cache/persistence smoke; and performance smoke at 1.25 ms tick work against the 8 ms ceiling, 1.53 s cold start, and 2.79 s 36-hole fixture load. Initial JavaScript is 1,604,905 / 1,608,719 bytes and initial critical transfer is 3,785,162 / 4,194,304 bytes.
- The frozen release candidate is certified by fresh compact and desktop browser shards in `/private/tmp/zk1240-full-matrix-final-frozen`; the release workflow owns exact-scope commit, development verification, production promotion, monitoring, and Linear completion for ZK-1240, ZK-1241, and ZK-1244.
...
- The 12-scene browser matrix now registers one Playwright test per fixture,
  giving every biome/rotation its own timeout, output, and failure boundary
  instead of sharing a single 10-minute clock. `ZK765_VISUAL_IDS` supports
  exact bounded smoke selections. Parkland r0 (including the 250-pointer
  diagnostic) and the former Links r2 timeout point pass independently in a
  two-fixture smoke; the coordinator will run the remaining scenes.
...
## ZK-758 mixed-tee participant snapshots — 2026-08-06

- Started from exact clean Wave 1 SHA `2d7432a514b5621a8c1cccdf4b1543106b239fcb` in the isolated `codex/zk758-mixed-tee` worktree. The original prompt at the top of this file remains unchanged.
- ChallengeGroupRound v2 now freezes a compact participant-local setup: routed holes, explicit tee/pin identity and points, par, stroke indexes, and rating/slope, alongside Course Handicap evidence, allowance, Playing Handicap, and per-hole strokes. Shared course identity, physical tiles/elevations, and rules remain one frozen round snapshot rather than being duplicated for every golfer.
- Individual gross/net stroke and match play, Stableford, skins, Nassau, and measured-contest shot evidence now consume each golfer's frozen card/physics setup. Net match applies the existing strokes-off-low authority after each tee-specific allowance; 9/18-hole and mixed-sign matrices are covered.
- Legacy v1 shared-setup rounds migrate to equivalent v2 participant evidence inside the existing decoder: ordinary cards retain a truthful 100% allowance, representable explicit Playing Handicap overrides derive a deterministic allowance, and unrated history receives the par/113 neutral fallback. Native v2 saves require rating evidence and reject allowance drift even when stored Playing Handicap/strokes were left self-consistent. The synchronous codec/text consumers are split from play authority without a runtime cycle, while the original module re-exports the public API.
- Final verification: TypeScript passes; the final five-file competition/save/player slice passes 120/120, including mixed-setup interruption, duplicate calls, withdrawal, skins carry, closest-to-pin, longest-drive, Nassau, migration, missing-rating, and allowance-drift coverage. Full tests pass 171/171 files (1,358 passed, one intentional skip) plus 5/5 audio audits; lint has zero errors and the exact existing 12 Hook warnings, with both i18n guards green.
- Isolated Playwright coverage passes 3/3 with clean console/page errors, and the mixed participant scorecard plus neutral gameplay canvases were visually inspected. Two consecutive production builds produced the identical `index-DxECAPsX.js` at 1,608,070 raw initial-JavaScript bytes, 649 bytes under the fixed budget; audio, service-worker, M35, Parkland, residency, transfer, and delivery audits all pass.

## ZK-674 adaptive atlas generation repair — 2026-08-08

- Reproduced the reopened failure in code: adaptive quality changes immediately reached independent Pixi effects while the newly requested biome/tier bundle was still loading. Terrain and dressing could therefore rebuild to flat procedural diamonds or empty fallbacks while retained structures and other layers still represented the prior atlas generation.
- Atlas loading now returns an explicit activated, fallback, or superseded result and advances a monotonic generation only after the complete base and optional overlay decision is ready. The renderer holds its last complete quality context through in-flight and rapidly superseded requests, then lets terrain, smooth surfaces/edges/water, structures, and dressing consume the new context together.
- Added bounded requested/rendered/active/residency/fallback diagnostics, active-versus-latest request IDs, per-layer generation evidence, camera/ground-cover tier evidence, and deterministic e2e-only quality and zoom controls. The browser regression now targets all three biomes, all supported quality tiers, delayed rapid reversals, zoom thresholds, flat-diamond screenshot analysis, assets, and console errors.
- Final automated evidence: TypeScript; 34/34 focused atlas, manifest, scene-host, landscape, M35 telemetry, and ground-cover tests; full CI at 183/183 files, 1,445 passing tests, one intentional skip, and 5/5 audio audits; lint/i18n with zero errors and the unchanged 12 Hook warnings; production build and all embedded audio, service-worker, M35, Parkland, residency, transfer, and delivery audits. Initial JavaScript is 1,599,759 bytes, 8,960 below the 1,608,719-byte limit. The performance smoke passes at 0.73 ms tick work against the 8 ms limit, 999 ms cold startup, and 863 ms fixture load. The isolated Parkland/Links/Desert browser matrix passes 3/3 across High/Medium/Low transitions and all threshold reversals with clean asset/console capture and flat-block ratios below the fixed 4% corruption limit. High, Medium, Low, and bundled-client Desert captures were visually inspected; structures and dressing remain present and no broad fallback diamond or mixed-biome surface appears.
