...
test("ZK-1200 captures canonical medium seams and high-detail green rotations", async ({ page }) => {
  ...
  const captures: Array<{ quality: "medium" | "high"; rotation: number; file: string; crop: string }> = [];
  ...
  for (let turn = 0; turn < 4; turn++) {
    ...
    captures.push({ quality: "medium", rotation, file, crop });
  }

  // Renderer integration consumes this packet's halo cells later. Preserve
  // detailed green evidence at two rotations now so that integration can
  // compare seams without changing this geometry worker's scope.
  for (const rotation of [0, 90]) {
    await page.keyboard.press("e");
    await page.evaluate(() => {
      window.__coursecraftTest!.setGraphicsQualityFixture("high");
      window.__coursecraftPixiTest!.focusTileForTest(39, 20, 2);
    });
    await expect.poll(() => page.evaluate(() => {
      const state = window.__coursecraftPixiTest!.rendererAtlasState() as unknown as {
        pathMaterialCrossSection: { quality: string; camera: { rotation: number; zoom: number } };
    ...
    const file = resolve(outputRoot, `zk1200-high-2x-r${rotation}.png`);
    const crop = resolve(outputRoot, `zk1200-high-2x-r${rotation}-crop.png`);
    await writeFile(file, body);
    await writeFile(crop, PNG.sync.write(cropped));
    captures.push({ quality: "high", rotation, file, crop });
  }
  ...
  await writeFile(
    ...
    `${JSON.stringify({ version: 2, courseHash: initialHash, captures }, null, 2)}\n`,
  );
});
