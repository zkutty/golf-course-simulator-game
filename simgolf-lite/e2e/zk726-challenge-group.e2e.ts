...
test("app render_game_to_text exposes the persisted ChallengeGroupRound carrier", async ({ page }, testInfo) => {
  ...
  expect(state.activeChallengeGroupRound).toMatchObject({
    ...
    golfers: [
      expect.objectContaining({ id: "rival-alex", controller: "ai", ball: expect.any(Object), scorecard: expect.any(Array), latestShot: expect.any(Object) }),
      expect.objectContaining({ id: "player-pro-726001", controller: "player", ball: expect.any(Object), handicap: expect.any(Object), equipment: expect.any(Object) }),
      expect.objectContaining({ id: "rival-blair", controller: "ai" }),
      expect.objectContaining({ id: "rival-devon", controller: "ai" }),
    ],
